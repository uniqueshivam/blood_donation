...
class nearBy_Bank_listing{
    ...
    public function getNearby_banks($latitude,$longitude)
    {
        $query = "SELECT *, 
        ( 3959 * acos( cos( radians('$latitude') ) * 
        cos( radians( b_latitude ) ) * 
        cos( radians( b_longitude ) - 
        radians('$longitude') ) + 
        sin( radians('$latitude') ) * 
        sin( radians( b_latitude ) ) ) ) 
        AS distance FROM blood_bank_info HAVING distance < '519' ORDER BY distance ASC LIMIT 0, 10";
